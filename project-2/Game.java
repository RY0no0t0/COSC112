...
// =============================================================================
    ...
    private void readInitialState (String initialStatePath,
				   String cellType,
				   String gridType) throws OffTheGridException {
	...
	_grid = Grid.create(gridType, rows, columns, cellType);
	...
	while (true) {
	    ...
	    Cell SettingCell = null;
        try {
            SettingCell = _grid.getCell(row, col);
        } catch (OffTheGridException ex) {
            Support.abort(row+", "+col+" at lne "+lineNumber+"is out of Grid Size");
        }
        SettingCell.makeAlive();
     ...
     **/
    public void play (int generations) throws OffTheGridException {
     ...
     **/ 
    public int getPopulation () throws OffTheGridException {
    ...
    public Cell getCell (int row, int column) throws OffTheGridException{
        ...
        try {
            ReturningCell = _grid.getCell(row, column);
        } catch (OffTheGridException e) {
            Support.abort(row+", "+column+" are outside of desired input of the Grid");
        }
