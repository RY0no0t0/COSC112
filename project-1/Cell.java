...
// =============================================================================
    ...
    private int countLiveNeighbors () {
        ...
        for (int i = -1;  i< 2; i+=1) {
            ...
            for  (int j= -1; j<2; j+=1) {
                int columncheck = _column + j;

                if ((rowcheck >= 0) && (columncheck >= 0) && (rowcheck < _grid.getRows()) && (columncheck < _grid.getColumns())) { 
                    if ((rowcheck != _row) || (columncheck != _column)){
                        if (_grid.getCell(rowcheck, columncheck).isAlive()) {
                            LiveNeighbor += 1;
                        }
                    }
                }
